...
- or a workstream execution directory that writes to shared durable locations
...
- a shared private human improvement item under `.work/learning/human/`
- a shared workstream rule candidate under `.work/learning/rule-candidates/`
...
- If invoked with an in-scope instruction:
  - use that instruction to bias the output toward skill, human item, rule candidate, multi-output, or evaluation
...
2. Resolve the durable shared destination:
   - if running inside a workstream execution directory, use the shared root derived from the current workstream context
...
5. Resolve transcript candidates when a task is involved or when the user explicitly asks for transcript-backed learning:
   ...
   - otherwise infer likely transcripts from workstream path, task timestamps, and task keywords
...
6. Determine what is worth preserving:
   - a reusable technical or workflow pattern for future workstream work
   - a human improvement topic about alignment, blockers, or better collaboration behavior
   - a workstream-specific rule candidate that may later belong in `AGENTS.md`
...
7. Inspect overlap before drafting:
   - inspect:
     ...
     - `.work/learning/human/` when a human item is plausible
     - `.work/learning/rule-candidates/` when a workstream rule is plausible
8. Decide the output:
   - `save-skill` for a reusable pattern
   - `save-human-item` for a private human improvement topic
   - `save-workstream-rule` for a stable shared rule candidate
...
10. For a human item:
   - create or update `.work/learning/human/<id>.md` in the shared root
...
11. For a workstream rule candidate:
   - create or update `.work/learning/rule-candidates/<id>.md`
   - use TOML frontmatter with:
     - `id`
     - `status`
     - `created`
...
12. If there is clear overlap:
   - prefer updating the existing learned skill, human item, or rule candidate
...
- Keep workstream rule candidates focused on one stable rule proposal.
- Human items are private by default.
- Workstream rule candidates are not active runtime rules.
- Do not create or update runtime config from this skill.
